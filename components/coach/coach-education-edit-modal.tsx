...
const educationSchema = z.object({
	institution: z.string().trim().min(1, "La institucion es requerida").max(200),
...
interface CoachEducationEditModalProps {
	entry?: EducationEntry;
}

export const CoachEducationEditModal = NiceModal.create(
	({ entry }: CoachEducationEditModalProps) => {
		...
		const createMutation = trpc.coach.addEducation.useMutation({
			onSuccess: () => {
		...
		const updateMutation = trpc.coach.updateEducation.useMutation({
			onSuccess: () => {
		...
		const deleteMutation = trpc.coach.deleteEducation.useMutation({
			onSuccess: () => {
		...
		const onSubmit = form.handleSubmit((data: EducationFormData) => {
			const payload = {
		...
		const handleDelete = () => {
			if (entry && confirm("Eliminar esta educacion?")) {
		...
		return (
			<ProfileEditSheet
				...
				subtitle="Registra tu formacion academica"
			...
			>
				<div className="space-y-6">
					<ProfileEditSection title="Informacion de la Institucion">
						<FormField
						...
						<ProfileEditGrid cols={2}>
							<FormField
								...
								render={({ field }) => (
									<FormItem asChild>
										<Field>
											<FormLabel>Titulo / Certificacion</FormLabel>
											<FormControl>
												<Input
													placeholder="Licenciatura, Tecnico, etc."
													{...field}
													value={field.value ?? ""}
												/>
											</FormControl>
								...
								render={({ field }) => (
									<FormItem asChild>
										<Field>
											...
											<FormControl>
												<Input
													placeholder="Educacion Fisica, Entrenamiento..."
													{...field}
													value={field.value ?? ""}
												/>
											</FormControl>
							...
							render={({ field }) => (
								<FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
									...
									<div className="space-y-1 leading-none">
										...
										<FormDescription>
											Marcar si aun estas estudiando aqui
										</FormDescription>
									</div>
								</FormItem>
							)}
							...
							render={({ field }) => (
								<FormItem asChild>
									<Field>
										...
										<FormControl>
											<Textarea
												placeholder="Informacion adicional sobre tu formacion..."
