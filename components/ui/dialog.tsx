	...
	return (
		<DialogPortal data-slot="dialog-portal">
			...
			<DialogPrimitive.Content
				...
				// Sharp & Bold: stronger border, sharper corners, elevated shadow
				className={cn(
					"data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-[50%] left-[50%] z-50 grid w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] gap-4 rounded-lg border-2 border-border bg-background fill-mode-forwards! p-6 shadow-xl duration-200 data-[state=closed]:animate-out data-[state=open]:animate-in sm:max-w-lg",
					className,
				)}
			...
			>
				...
				{showCloseButton && (
					<DialogPrimitive.Close
						...
						// Sharp & Bold: hover background, faster transition
						className="absolute top-4 right-4 cursor-pointer rounded-md p-1.5 opacity-70 ring-offset-background transition-all duration-150 hover:opacity-100 hover:bg-muted focus:outline-hidden focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0"
						onClick={handleClose}
					>
						<XIcon />
