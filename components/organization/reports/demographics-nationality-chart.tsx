...
export function DemographicsNationalityChart(): React.JSX.Element {
	...
	const chartData = (data?.nationality ?? []).map((item) => ({
		...
		<Card>
			<CardHeader>
				<CardTitle className="flex items-center gap-2">
					<GlobeIcon className="h-5 w-5 text-cyan-600" />
					{t("charts.nationality.title")}
				</CardTitle>
				<CardDescription>{t("charts.nationality.description")}</CardDescription>
			</CardHeader>
			<CardContent>
				{chartData.length === 0 ? (
