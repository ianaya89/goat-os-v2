...
export function DemographicsPositionChart(): React.JSX.Element {
	...
	const chartData = (data?.position ?? []).map((item) => ({
		...
		<Card>
			<CardHeader>
				<CardTitle className="flex items-center gap-2">
					...
					{t("charts.position.title")}
				</CardTitle>
				<CardDescription>{t("charts.position.description")}</CardDescription>
			</CardHeader>
			<CardContent>
				{chartData.length === 0 ? (
