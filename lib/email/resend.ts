...
import { logEmailToConsole } from "./dev-console";
 ...
 * In development mode, if Resend is not configured, logs email to console instead.
 */
export async function sendEmail(
	...
	// In dev mode without email config, log to console instead of sending
	if (isDev && !hasEmailConfig()) {
		const mockFrom = from || "dev@localhost";
		logEmailToConsole(payload, mockFrom);

		logger.info("Email logged to console (dev mode, Resend not configured)", {
