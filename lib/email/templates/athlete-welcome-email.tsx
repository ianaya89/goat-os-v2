...
function AthleteWelcomeEmail({
	...
	const preview = t.athleteWelcome.preview
		.replace("{organizationName}", organizationName)
		.replace("{appName}", appName);

	const title = t.athleteWelcome.title.replace(
		"{organizationName}",
		organizationName,
	);
	...
	return (
		...
		>
			...
			<Text className="text-[14px] text-black leading-[24px]">
				{t.athleteWelcome.body
					.replace("{organizationName}", organizationName)
					.replace("{appName}", appName)}
			</Text>
			<Text className="text-[14px] text-black leading-[24px]">
				{t.athleteWelcome.accountCreated}
			</Text>
			<Section
			...
			>
				<Text className="m-0 text-[14px] text-black leading-[28px]">
					<strong>1.</strong> {t.athleteWelcome.steps.step1}
					<br />
					<strong>2.</strong> {t.athleteWelcome.steps.step2}
					<br />
					<strong>3.</strong> {t.athleteWelcome.steps.step3}
					<br />
					<strong>4.</strong> {t.athleteWelcome.steps.step4}
				</Text>
			</Section>
			<Section className="my-[32px] text-center">
				<EmailButton href={forgotPasswordUrl}>
					{t.athleteWelcome.button}
				</EmailButton>
			</Section>
			<EmailLinkFallback href={forgotPasswordUrl} label={t.common.orCopyLink} />
			<Text className="text-[14px] text-black leading-[24px]">
				{t.athleteWelcome.loginInfo}{" "}
				<Link
					href={loginUrl}
...
AthleteWelcomeEmail.PreviewProps = {
	...
	t: {
		...
		athleteWelcome: {
			...
			body: "Fuiste agregado como Atleta a la organizacion {organizationName} en {appName}.",
			accountCreated:
