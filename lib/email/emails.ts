...
export async function sendCoachWelcomeEmail(
	...
	await sendEmail({
		recipient: input.recipient,
		subject: `Welcome to ${input.organizationName} - Your coach account is ready`,
		html,
		text,
	});
}
...
export async function sendAthleteWelcomeEmail(
	input: AthleteWelcomeEmailProps & { recipient: string },
): Promise<void> {
	const { AthleteWelcomeEmail } = await import(
		"./templates/athlete-welcome-email"
	);
	const component = AthleteWelcomeEmail(input);
	const html = await render(component);
	const text = await render(component, { plainText: true });

	await sendEmail({
		recipient: input.recipient,
		subject: `Welcome to ${input.organizationName} - Your athlete account is ready`,
		html,
		text,
	});
}
